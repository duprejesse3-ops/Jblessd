...
-- 222.7 km, -118.3 km). Also scanned for eval/exec/subprocess/network
-- calls — none found; it's a self-contained, offline computation tool.
--
...
INSERT INTO products (sku, name, category, niche, format, price, blurb, spec) VALUES
  ('AI-AG-067', 'MultiBøT', 'agents', 'developers', '.zip · Python tool layer + Agent Studio config', 49, 'A coding bot that plans vehicle routes, then proposes structurally different alternatives — with real libraries, not three near-identical heuristics. One model, one hat at a time: Architect, Builder, Builder, Critic. The critic refuses to ship if the alternatives are just heuristic wobble.', 'NetworkX + optional OR-Tools/OSMnx · baseline -> named hypotheses -> re-solve -> defended alternative')
