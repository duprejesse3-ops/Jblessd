...
**file-organizer-agent — perpetual single-purchase license**
...
- Modify the source freely. It is plain JavaScript with no build step precisely so
  that you can — add your own categories, change the destination structure,
  anything.
...
In particular: this software moves and renames files on whatever folder you point
it at. `--dry-run` is the default specifically so you can review a plan before
anything moves; you are responsible for reviewing that plan, for the folder you
choose to run `--apply` against, and for keeping your own backups. The optional
`--ai` flag sends filenames (never file contents) to a third-party model
provider — you are responsible for your own API usage and any cost it incurs.
