...
**meridian-gate — perpetual single-purchase license**
...
- Modify the source freely. It is plain JavaScript with no build step and zero
  dependencies precisely so that you can.
...
`npm test` before you ask — it takes a few seconds, runs real sockets including
an end-to-end proxy tunnel, and tells you whether the software is at fault.
...
In particular: this tool decides whether an outbound connection is allowed to
open — it is one control among several you should run (a host firewall, careful
handling of API keys, review of what your own code and its dependencies do).
It does not inspect traffic inside an allowed tunnel, does not replace a
firewall, and does not guarantee that a determined process on a compromised
machine cannot reach the network by some other path. You are responsible for
deciding what belongs on the allowlist and for the consequences of allowing it.
