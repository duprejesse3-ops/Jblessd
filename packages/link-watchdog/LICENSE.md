...
**link-watchdog — perpetual single-purchase license**
...
You bought it once. You own your copy forever. Run it against as many of your
**own** sites and sitemaps as you like. Do not resell it as a product of its
own.

## What you may do

- Use the software for any purpose, commercial or personal, forever.
- Run it against unlimited sites, sitemaps, and pages that you own or
  operate.
- Use it in client work, and use the reports it produces however you like.
...
Selling the *output* — your own site-monitoring or SEO-maintenance service,
consulting on site health — is explicitly allowed. Selling *the tool* is not.
...
In particular: this tool fetches URLs listed in a sitemap you configure, and —
if you enable it — outbound links found on those pages, including pages and
servers you do not control. You are responsible for pointing it only at sites
you are authorized to crawl, for the request volume and rate it generates
against your own and third-party servers, and for your own API keys and their
associated costs. It does not modify anything on the sites it checks, place or
remove any content, or guarantee that a "broken" classification is accurate for
every server configuration (some servers block automated HEAD/GET requests
outright, which this tool cannot distinguish from an actually broken page) —
you are responsible for reviewing flagged results before acting on them.
