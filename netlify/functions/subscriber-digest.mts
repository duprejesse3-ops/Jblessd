...
export default async () => {
  ...
  const text =
    `Here's what's new at MULTI-VICE AI — ready-to-run AI tools you can watch work before you buy.\n\n` +
    `${lines}\n\n` +
    `Browse everything, or run any tool live on your own task first: ${SITE}\n\n` +
    `— The MULTI-VICE AI team\n\n` +
    `You're getting this because you grabbed a free prompt pack at jblessd.com. Reply to unsubscribe.`

  const subject = `New at MULTI-VICE AI: ${featured[0].name} and more`
