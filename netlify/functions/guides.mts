...
export default async (req: Request, _context: Context) => {
  ...
  // Lightweight mode: just the slugs, no title/body_html payload. Used by
  // sitemap.mts, which only needs to enumerate /guides/:slug URLs — see the
  // matching mode on /api/proof for why this matters once this list grows.
  if (url.searchParams.get('slugs') === '1') {
    try {
      const db = getDatabase()
      const rows = (await db.sql`
        SELECT slug FROM seo_pages WHERE status = 'published' ORDER BY published_at DESC NULLS LAST, generated_at DESC
      `) as Array<{ slug: string }>
      return Response.json({ slugs: (rows ?? []).map((r) => r.slug) }, { headers: READ_CACHE })
    } catch (err) {
      console.error('guides slugs GET error:', (err as Error).message)
