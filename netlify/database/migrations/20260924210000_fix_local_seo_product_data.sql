-- Corrective migration — see README for why this exists.
--
-- The migration "20260924190000_add_time_saved_and_local_seo_product" was
-- applied to production once already, then its file was edited afterward
-- (price/blurb/em-dash fixes) before being pushed again. Netlify Database
-- checksums applied migrations and refuses to reapply a changed one — that
-- checksum mismatch is what just failed the build. Migrations are
-- immutable once applied; the fix is to leave that file as whatever was
-- actually applied (restored via `netlify database migrations pull` — see
-- README) and bring the DATA to the correct final state here instead, in a
-- new migration.
--
-- This uses UPSERT (ON CONFLICT ... DO UPDATE), not DO NOTHING, specifically
-- so it corrects the row to the values below regardless of which draft of
-- the original migration actually ran — whether AI-AB-071 was never
-- inserted, inserted with an earlier price, or inserted with the
-- double-escaped em-dash bug. Every UPDATE and the ADD COLUMN are already
-- idempotent on their own and are safe to run again.
