...
const SKU_APPS: Record<string, (p: Product) => ProductApp> = {
  ...
  'AI-AB-071': (product) => ({
    ...
    fields: [
      ...
      {
        id: 'category',
        label: 'What kind of business is it? (optional, sharpens the fixes)',
        type: 'text',
        placeholder: 'e.g. Plumbing, Dental practice, Coffee shop',
      },
    ],
  }),
}
...
export function buildRunPrompt(
  ...
  const user =
    ...
    (fixPack ? `Fix pack, generated deterministically by code from the live context above (not by you):\n${fixPack}\n\n` : '') +
    `The buyer's input:\n${filled}\n\n` +
