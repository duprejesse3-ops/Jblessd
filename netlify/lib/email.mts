// Shared transactional-email sender for MULTI-VICE AI.
//
...
//   EMAIL_FROM      — the verified From address, e.g. "MULTI-VICE AI <hello@jblessd.com>"
//
...
function wrapHtml(bodyText: string): string {
  ...
  return (
    ...
    `<div style="background:#000803;padding:18px 28px"><span style="color:#00FF41;font-weight:700;letter-spacing:.04em">MULTI-VICE AI</span></div>` +
    `<div style="padding:28px">${paragraphs}</div>` +
    `<div style="padding:16px 28px;border-top:1px solid #eef2ef;font-size:12px;color:#6b7d70">` +
    `MULTI-VICE AI · <a href="https://jblessd.com" style="color:#0a7d2c">jblessd.com</a></div>` +
    `</div></div>`
  )
}
