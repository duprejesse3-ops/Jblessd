...
export const SKU_RUN_BRIEF: Record<string, string> = {
  ...
  'AI-AG-067':
    'The buyer owns the source of MultiBøT, a Python vehicle-routing tool (multibot.py running the Architect/Builder/Builder/Critic cascade; main.py for a solver-only run with no commentary), and needs it set up for their own stop list. Return a concrete setup plan, not a live solve — you have not run their data, and must not imply you have or invent a result for it. Cover: (1) the exact command with real flags filled in from what they described — python3 multibot.py --stops <their_file>.csv --vehicles <N> --mode haversine (or --mode osm if they want road-true distances and have OSMnx installed; say plainly that osm mode needs the optional OSMnx dependency from requirements.txt, not the default install). (2) Their CSV needs an id,lat,lon header with the depot as the first row — if what they described is not already in that shape (a spreadsheet with different column names, addresses with no coordinates yet), say exactly what needs to change before the tool can read it; do not claim a geocoding step exists if they only have street addresses, since this kit takes lat/lon, not addresses. (3) If they gave fleet capacity or time-window constraints, point them at reasoning_loop.py\'s hypothesis step, where those become named model mutations (not a heuristic swap) — describe in plain language what a hypothesis reflecting their actual constraint would look like, without inventing route numbers for their specific stops. (4) If OR-Tools is not mentioned as installed, note it is optional per requirements.txt and the local fallback solver runs without it. Never present a plan as having already produced a route, a distance figure, or a km savings number for their data — those only exist once they actually run the command themselves.',
  'AI-AG-111':
