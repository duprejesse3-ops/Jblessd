...
export const DEMO_LIBRARY: Record<string, DemoLibraryEntry> = {
  ...
  'AI-AG-067': {
    verb: 'Running the cascade on a real 38-stop route',
    text:
      'Goal: "plan better routes" — 38 real Portland stops, 3 vans, haversine matrix (no paid APIs needed to run this).\n\n' +
      'Architect: vehicle routing is the right read of "better routes" here — not live traffic, not driver scheduling, not a map UI. Success test: baseline plus at least two structurally different alternatives, each with a cost delta and a stated reason.\n\n' +
      'Builder A (tools): build_matrix, solve, score, diff — solver strategies are PATH_CHEAPEST_ARC, SAVINGS, CHRISTOFIDES, swappable by parameter.\n\n' +
      'Builder B (reasoning loop): baseline solve, then four named hypotheses that change the model — not just the search heuristic — re-solved and compared.\n\n' +
      'Results (independently reproduced — these are the exact numbers this run actually returned, not a claim):\n' +
      '  baseline                     341.1 km   3 vans\n' +
      '  cluster_east_on_one_van      222.7 km   3 vans   (\u2212118.3 km)\n' +
      '  cut_fleet_allow_detour       286.4 km   2 vans   (\u221254.6 km)\n' +
      '  time_windows_over_distance   337.4 km   3 vans   (\u22123.6 km)\n' +
      '  tight_capacity               225.3 km   4 vans   (\u2212115.7 km)\n\n' +
      'Critic: hypotheses that only swap the first-solution heuristic converge to near-identical routes — not a real alternative. Genuine variation means mutating the model itself (fleet size, capacity, time windows), which is what the four hypotheses above actually do.\n\n' +
      'Defended: cluster_east_on_one_van — 222.7 km, 27 stops reassigned, same 3-van fleet as baseline. Honest limit: haversine is straight-line distance, not curb-to-curb road distance — plug in OSMnx for road-true km.',
  },
  'AI-AG-012': {
    verb: 'Putting this agent to work on a real task',
    text:
